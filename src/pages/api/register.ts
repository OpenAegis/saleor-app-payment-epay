 ...
 * 考虑CDN和反向代理的情况
 */
function getRealClientIP(req: { headers: { [key: string]: string | string[] | undefined } }): string | null {
  try {
    ...
    if (forwarded) {
      ...
      const forwardedStr = typeof forwarded === "string" ? forwarded : Array.isArray(forwarded) ? forwarded[0] : null;
      if (forwardedStr) {
    ...
    logger.warn({ error: error instanceof Error ? error.message : "未知错误" }, "提取客户端IP时出错");
  }
...
const baseHandler = createAppRegisterHandler({
  ...
  async onRequestVerified(req, { authData }) {
    ...
    try {
      ...
      const isSiteAuthorized = await siteManager.isAuthorized(saleorDomain, realClientIP || undefined);
      if (!isSiteAuthorized) {
      ...
      if (!existingSite) {
        ...
        if (realClientIP) {
          ...
          await siteManager.update(registeredSite.id, {
            notes: `真实客户端IP: ${realClientIP}${registeredSite.notes ? ` | ${registeredSite.notes}` : ""}`
          });
        }
        ...
        if (realClientIP) {
          logger.info(`更新站点备注，添加真实客户端IP: ${realClientIP}`);
          const updatedNotes = `真实客户端IP: ${realClientIP}${existingSite.notes ? ` | ${existingSite.notes}` : ""}`;
      ...
      if (whitelist.length > 0) {
        ...
        if (!isAllowed) {
          logger.warn(`域名和IP都不在白名单中，需要审核: 域名=${saleorDomain}, IP=${realClientIP || "未知"}`);
          // 注意：这里我们不直接返回错误响应，因为这是Saleor的回调处理
          // 白名单检查应该在注册流程的其他部分进行
        }
      }
    } catch (error) {
      logger.error({ error: error instanceof Error ? error.message : "未知错误" }, "处理Saleor回调时出错");
    }
  },
});
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    try {
      ...
      logger.error({ error: error instanceof Error ? error.message : "未知错误" }, "数据库初始化失败");
      // 继续执行，可能数据库已经存在
    }
