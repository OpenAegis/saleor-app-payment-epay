...
export class EpayClient {
  ...
  private async createOrderV2(params: CreateOrderParams): Promise<EpayOrderResult> {
    ...
    const recommendedMethod =
      params.method || EpayClient.getRecommendedMethod(detectedDevice, params.payType);
  ...
  private generateSign(params: Record<string, string>): string {
    ...
    console.log("[EpayClient] 签名生成详情", {
      filteredParams: Object.keys(params).filter(
        (k) => k !== "sign" && k !== "sign_type" && params[k],
      ),
      sortedParams,
      signType,
      keyLength:
        signType === "MD5" ? this.config.key.length : this.config.rsaPrivateKey?.length || 0,
      signString:
        signType === "MD5"
          ? (sortedParams + this.config.key).substring(0, 50) + "..."
          : sortedParams,
      generatedSign: sign,
    });
  ...
  async queryOrder(tradeNoOrOutTradeNo: string, useOutTradeNo = false): Promise<EpayQueryResult> {
    ...
    // v2 API 需要 timestamp 参数
    if (apiVersion === "v2") {
      requestData.timestamp = Math.floor(Date.now() / 1000).toString();
    }
    ...
    // v2 API 使用 POST 请求，v1 API 使用 GET 请求
    if (apiVersion === "v2") {
      // v2 API 使用 POST 方法
      try {
        // 确保正确的 URL 拼接，避免双斜杠问题
        const baseUrl = this.config.apiUrl.replace(/\/+$/, ""); // 移除尾部斜杠
        const queryEndpoint = `${baseUrl}/api/pay/query`;
